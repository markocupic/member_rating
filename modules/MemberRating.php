...
abstract class MemberRating extends \Module
{
	...
	public static function getScore($id)
	{
		...
		if(!isset(self::$cache['score'][$id]))
		{
			...
			self::$cache['score'][$id] = $objPoints->sumscore <= 0 ? '0' : $objPoints->sumscore;
		}
		return self::$cache['score'][$id];

	}


	/**
	 * get rating enities (Anzahl Bewertungen)
	 *
	 * @param $id
	 * @return mixed
	 */
	public static function getRatingEnities($id)
	{
		if(!isset(self::$cache['ratingEnities'][$id]))
		{
			$objEnities = \Database::getInstance()
								   ->prepare("SELECT count(id) as enities FROM tl_comments WHERE source = ? AND parent = ? AND owner > ?")
								   ->execute('tl_member', $id, 0);
			self::$cache['ratingEnities'][$id] = $objEnities->enities <= 0 ? '0' : $objEnities->enities;
		}
		return self::$cache['ratingEnities'][$id];
	...
	{
		if(self::getRatingEnities($id) > 0)
		{
			$avg = self::getScore($id) / self::getRatingEnities($id);
		}
		else
		{
			$avg = '0';
	...
	public static function getAverageScore($id)
	{
		...
		$objPoints = \Database::getInstance()
							  ->prepare("SELECT count(id) as sumscore FROM tl_comments WHERE source = ? AND parent = ? AND owner > ?")
							  ->execute('tl_member', $id, 0);
